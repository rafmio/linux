...
char* shared_array;      // указатель на разделяемый массив
size_t shared_array_len; // размер разделяемого массива
...
void*
alloc_thread_body(void* arg) {
    printf("alloc_thread_body: %p\n", arg);
    shared_array_len = 20;
...
void*
filler_thread_body(void* arg) {
    printf("filler_thread_body: %p\n", arg);
    pthread_barrier_wait(&alloc_barrier);
    int even = *((int*)arg);
    char c = 'a';
...
void*
printer_thread_body(void* arg) {
    printf("printer_thread_body: %p\n", arg);
    pthread_barrier_wait(&fill_barrier);
    printf(">> %s\n", shared_array);
...
void*
dealloc_thread_body(void* arg) {
    printf("dealloc_thread_body: %p\n", arg);
    pthread_barrier_wait(&done_barrier);
    free(shared_array);
...
main() {
    ...
    res = pthread_create(&even_filler_thread, &attr, filler_thread_body, NULL);
    CHECK_RESULT(res);

    res = pthread_create(&odd_filler_thread,  &attr, filler_thread_body, NULL);
    CHECK_RESULT(res);
